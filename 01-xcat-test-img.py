...
parser.add_argument("--model_path", type=str, default='checkpoints/xcat/ckpt_271')
parser.add_argument("--data_path", type=str, default='test-imgs/xcat')
...
for root, _, files in os.walk(args.data_path):
    for file in files:
        ...
        img_noise = np.fromfile(img_noise_path, dtype=np.float32).reshape(256, 256, 24).transpose(2, 0, 1)
